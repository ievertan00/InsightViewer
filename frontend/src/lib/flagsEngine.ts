...
export const analyzeFlags = (reports: any[]): FlagResult[] => {
  ...
  const tradingFinAssets =
    getVal(d, "balance_sheet.current_assets.trading_financial_assets") +
    getVal(
      d,
      "balance_sheet.current_assets.financial_assets_fvpl.trading_financial_assets"
    );
  const arTotal = getVal(
    d,
  ...
  const otherReceivables = getVal(
    d,
    "balance_sheet.current_assets.other_receivables_total.amount"
  );
  ...
  const longTermDeferredExp = getVal(
    d,
  ...
  const prevPrepayments = getVal(
    pd,
    "balance_sheet.current_assets.prepayments"
  );
  const prevSellingExp = getVal(
    pd,
    "income_statement.total_operating_cost.selling_expenses"
  );
  const prevAdminExp = getVal(
    pd,
    "income_statement.total_operating_cost.admin_expenses"
  );
  const prevPayables = getVal(
    pd,
    "balance_sheet.current_liabilities.notes_and_accounts_payable.amount"
  );
  const prevCash = getVal(pd, "balance_sheet.current_assets.monetary_funds");
  const prevTotalDebt =
    getVal(pd, "balance_sheet.current_liabilities.short_term_borrowings") +
  ...
  const prevOpMargin = safeDiv(
    getVal(pd, "income_statement.operating_profit.amount"),
    prevRevenue
  );
  const niGrowth = safeGrowth(
    netProfitParent,
    getVal(pd, "income_statement.net_profit.net_profit_attr_to_parent")
  );
  const ocfGrowth = safeGrowth(
    ocf,
    getVal(
      pd,
      "cash_flow_statement.operating_activities.net_cash_flow_from_operating"
  ...
  const prevFCF =
    ...
    getVal(pd, "cash_flow_statement.investing_activities.cash_paid_for_assets");
  const annPrevFCF = prevFCF;
  ...
  flags.push({
    ...
    category: "Cost",
    type: "Red",
    status: gmIncrease > 0.05 && invTurnoverChange < -0.1,
    value: `GM Inc: ${(gmIncrease * 100).toFixed(1)}%, Inv Turn Chg: ${(
      invTurnoverChange * 100
    ).toFixed(1)}%`,
    threshold: "GM Inc > 5% & Inv Turn Dec > 10%",
    logic: `Gross Margin Increase = ${(gmIncrease * 100).toFixed(
      1
    )}% (> 5%) & Inventory Turnover Change = ${(
      invTurnoverChange * 100
    ).toFixed(1)}% (< -10%)`,
    description:
  ...
  flags.push({
    ...
    category: "Asset",
    type: "Red",
    status: inventoryGrowth - revenueGrowth > 0.2,
    value: `Inv Gr: ${(inventoryGrowth * 100).toFixed(1)}%, Rev Gr: ${(
      revenueGrowth * 100
    ).toFixed(1)}%, Gap: ${((inventoryGrowth - revenueGrowth) * 100).toFixed(
      1
    )}%`,
  ...
  // ID: F3
  const begininterestpayable = pd
    ? getVal(
        pd,
        "balance_sheet.current_liabilities.other_payables_total.interest_payable"
      )
    : 0;
  const endinterestpayable = getVal(
    d,
    "balance_sheet.current_liabilities.other_payables_total.interest_payable"
  );
  const interestPaidEstim =
    interestExp + begininterestpayable - endinterestpayable;

  const annProxyInterestPaid = interestPaidEstim * flowMult;
  const proxyCashInterestRate = safeDiv(
    annProxyInterestPaid,
    avgInterestBearingDebt
  );
  flags.push({
    ...
    logic: `Annualized Cash Interest Paid / Avg Debt = ${(
      proxyCashInterestRate * 100
    ).toFixed(1)}% (> 5%)`,
    description:
  ...
  flags.push({
    ...
    threshold: "< 0.5%",
    logic: `Annualized Interest Income / Avg Cash = ${(
      depositRate * 100
    ).toFixed(2)}% (< 0.5%)`,
    description:
  ...
  // ID: F5
  const hchd_c1 =
    safeDiv(cash, shortTermDebt + tradingFinLiab) >= 1.0 && shortTermDebt > 0;
  const hchd_c2 = safeDiv(interestBearingDebt, totalAssets) >= 0.3;
  const hchd_c3 = depositRate < 0.005;
  const hchd_c4 =
    safeDiv(cashGrowth, revenueGrowth) >= 2.0 && revenueGrowth > 0;
  ...
  flags.push({
    ...
    category: "Earnings",
    type: "Red",
    status: safeDiv(assetDisposalIncome, netProfit) > 0.2 && netProfit > 0,
    value: `Ratio: ${(safeDiv(assetDisposalIncome, netProfit) * 100).toFixed(
      1
    )}%`,
    threshold: "> 20% of Total Net Profit",
    logic: `Asset Disposal Income / Total Net Profit = ${(
      safeDiv(assetDisposalIncome, netProfit) * 100
    ).toFixed(1)}% (> 20%)`,
    description:
  ...
  flags.push({
    ...
    type: "Red",
    status:
      safeDiv(totalAssetImpairment, operatingProfit) > 0.2 &&
      operatingProfit > 0,
    value: `Ratio: ${(
      safeDiv(totalAssetImpairment, operatingProfit) * 100
    ).toFixed(1)}%`,
    threshold: "> 20% of Op Profit",
    logic: `Asset Impairment / Op Profit = ${(
      safeDiv(totalAssetImpairment, operatingProfit) * 100
    ).toFixed(1)}% (> 20%)`,
    description:
  ...
  flags.push({
    ...
    type: "Red",
    status:
      safeDiv(totalCreditImpairment, operatingProfit) > 0.15 &&
      operatingProfit > 0,
    value: `Ratio: ${(
      safeDiv(totalCreditImpairment, operatingProfit) * 100
    ).toFixed(1)}%`,
    threshold: "> 15% of Op Profit",
    logic: `Credit Impairment / Op Profit = ${(
      safeDiv(totalCreditImpairment, operatingProfit) * 100
    ).toFixed(1)}% (> 15%)`,
    description:
  ...
  flags.push({
    ...
    category: "Earnings",
    type: "Red",
    status: safeDiv(nonOperatingRev, totalProfit) > 0.2 && totalProfit > 0,
    value: `Ratio: ${(safeDiv(nonOperatingRev, totalProfit) * 100).toFixed(
      1
    )}%`,
    threshold: "> 20% of Total Profit",
    logic: `Non-op Revenue / Total Profit = ${(
      safeDiv(nonOperatingRev, totalProfit) * 100
    ).toFixed(1)}% (> 20%)`,
    description:
  ...
  flags.push({
    ...
    category: "Earnings",
    type: "Red",
    status: safeDiv(fairValueChange, netProfit) > 0.3 && netProfit > 0,
    value: `Ratio: ${(safeDiv(fairValueChange, netProfit) * 100).toFixed(1)}%`,
    threshold: "> 30% of Total Net Profit",
    logic: `Fair Value Income / Total Net Profit = ${(
      safeDiv(fairValueChange, netProfit) * 100
    ).toFixed(1)}% (> 30%)`,
    description:
  ...
  flags.push({
    ...
    type: "Red",
    status:
      safeDiv(investmentIncome, operatingProfit) > 0.3 && operatingProfit > 0,
    value: `Ratio: ${(safeDiv(investmentIncome, operatingProfit) * 100).toFixed(
      1
    )}%`,
    threshold: "> 30% of Op Profit",
    logic: `Inv Income / Op Profit = ${(
      safeDiv(investmentIncome, operatingProfit) * 100
    ).toFixed(1)}% (> 30%)`,
    description:
  ...
  flags.push({
    ...
    category: "Earnings",
    type: "Red",
    status: safeDiv(otherIncome, operatingProfit) > 0.3 && operatingProfit > 0,
    value: `Ratio: ${(safeDiv(otherIncome, operatingProfit) * 100).toFixed(
      1
    )}%`,
    threshold: "> 30% of Op Profit",
    logic: `Other Income / Op Profit = ${(
      safeDiv(otherIncome, operatingProfit) * 100
    ).toFixed(1)}% (> 30%)`,
  ...
  flags.push({
    ...
    category: "Asset",
    type: "Red",
    status: arToAssets > 0.3 || arGrowthGap > 0.15,
    value: `AR/Asset: ${(arToAssets * 100).toFixed(1)}%, AR Gr: ${(
      arGrowth * 100
    ).toFixed(1)}%, Rev Gr: ${(revenueGrowth * 100).toFixed(1)}%, Gap: ${(
      arGrowthGap * 100
    ).toFixed(1)}%`,
    threshold: "AR/Asset > 30% OR Gap > 15%",
    logic: `Ratio: ${(arToAssets * 100).toFixed(
      1
    )}% (> 30%) OR AR Gr - Rev Gr = ${(arGrowthGap * 100).toFixed(1)}% (> 15%)`,
    description:
  ...
  flags.push({
    ...
    category: "Asset",
    type: "Red",
    status: invToAssets > 0.3 || invGrowthGap > 0.15,
    value: `Inv/Asset: ${(invToAssets * 100).toFixed(1)}%, Inv Gr: ${(
      inventoryGrowth * 100
    ).toFixed(1)}%, Rev Gr: ${(revenueGrowth * 100).toFixed(1)}%, Gap: ${(
      invGrowthGap * 100
    ).toFixed(1)}%`,
    threshold: "Inv/Asset > 30% OR Gap > 15%",
    logic: `Ratio: ${(invToAssets * 100).toFixed(
      1
    )}% (> 30%) OR Inv Gr - Rev Gr = ${(invGrowthGap * 100).toFixed(
      1
    )}% (> 15%)`,
    description:
  ...
  flags.push({
    ...
    logic: `Other Rec / Total Assets = ${(otherRecToAssets * 100).toFixed(
      1
    )}% (> 5%)`,
    description:
  ...
  flags.push({
    ...
    category: "Asset",
    type: "Red",
    status: goodwillToNetAssets > 0.2,
    value: `Ratio: ${(goodwillToNetAssets * 100).toFixed(1)}%`,
    threshold: "> 20% of Net Assets",
    logic: `Goodwill / Net Assets = ${(goodwillToNetAssets * 100).toFixed(
      1
    )}% (> 20%)`,
    description:
  ...
  flags.push({
    ...
    threshold: "> 5% of Total Assets",
    logic: `Deferred Exp / Total Assets = ${(deferredExpToAssets * 100).toFixed(
      1
    )}% (> 5%)`,
  ...
  // ID: F20
  const sellingEffGap = revenueGrowth - 0.2;
  flags.push({
    name: "Selling Expense Efficiency",
    category: "Cost",
    type: "Red",
    status: sellingExpGrowth < sellingEffGap && revenueGrowth > 0.1,
    value: `Sell Gr: ${(sellingExpGrowth * 100).toFixed(1)}%, Rev Gr: ${(
      revenueGrowth * 100
    ).toFixed(1)}%, Gap: ${((sellingExpGrowth - revenueGrowth) * 100).toFixed(
      1
    )}%`,
    threshold: "Sell Exp Gr < Rev Gr - 20%",
    logic: `Sell Gr < Rev Gr - 20% (Gap: ${(
      (sellingExpGrowth - revenueGrowth) *
      100
    ).toFixed(1)}% < -20%)`,
    description:
  ...
  flags.push({
    ...
    value: `Admin Gr: ${(adminExpGrowth * 100).toFixed(1)}%, Rev Gr: ${(
      revenueGrowth * 100
    ).toFixed(1)}%, Gap: ${(adminDivGap * 100).toFixed(1)}%`,
    threshold: "(Admin Gr - Rev Gr) > 15%",
    logic: `Admin Gr - Rev Gr = ${(adminDivGap * 100).toFixed(
      1
    )}% (Threshold: > 15%)`,
    description:
  ...
  flags.push({
    ...
    category: "Asset",
    type: "Red",
    status: prepayToAssets > 0.05 || prepaymentGrowth > 0.5,
    value: `Ratio: ${(prepayToAssets * 100).toFixed(1)}%, Gr: ${(
      prepaymentGrowth * 100
    ).toFixed(1)}%`,
    threshold: "> 5% Assets OR > 50% Growth",
    logic: `Prepay/Assets = ${(prepayToAssets * 100).toFixed(
      1
    )}% (> 5%) OR Growth = ${(prepaymentGrowth * 100).toFixed(1)}% (> 50%)`,
    description:
      "Massive prepayments are common vehicles for embezzlement or fund transfer.",
  });

  // ID: F23
  const payablesGap = payablesGrowth - (revenueGrowth + 0.2);
  flags.push({
    name: "Payables Gap",
    category: "Liquidity",
    type: "Red",
    status: payablesGrowth > revenueGrowth + 0.2,
    value: `Pay Gr: ${(payablesGrowth * 100).toFixed(1)}%, Rev Gr: ${(
      revenueGrowth * 100
    ).toFixed(1)}%, Gap: ${(payablesGap * 100).toFixed(1)}%`,
    threshold: "Payables Gr > Rev Gr + 20%",
    logic: `Pay Gr - Rev Gr = ${(payablesGap * 100).toFixed(
      1
    )}% (Threshold: > 20%)`,
    description:
  ...
  flags.push({
    ...
    category: "Earnings",
    type: "Red",
    status: otherCashRecRatio > 0.1,
    value: `Ratio: ${(otherCashRecRatio * 100).toFixed(1)}%`,
    threshold: "> 10% of Op Inflow",
    logic: `Other Op Cash In / Total Op Inflow = ${(
      otherCashRecRatio * 100
    ).toFixed(1)}% (> 10%)`,
  ...
  flags.push({
    ...
    threshold: "> 1.1",
    logic: `OCF / Total Net Profit = ${ocfNiRatio.toFixed(
      2
    )} (threshold > 1.1)`,
    description:
  ...
  flags.push({
    ...
    threshold: "< 2.0x",
    logic: `Net Debt / Annualized EBITDA = ${leverageRatio.toFixed(
      2
    )} (threshold < 2.0x)`,
    description:
  ...
  const totalNiGrowth = safeGrowth(
    netProfit,
    getVal(pd, "income_statement.net_profit.amount")
  );
  flags.push({
    ...
    value: `NI Growth: ${(totalNiGrowth * 100).toFixed(1)}%, OCF Growth: ${(
      ocfGrowth * 100
    ).toFixed(1)}%`,
    threshold: "NI > 10% & OCF <= 0%",
    logic: `Total NI Growth = ${(totalNiGrowth * 100).toFixed(
      1
    )}% (threshold > 10%) & OCF Growth = ${(ocfGrowth * 100).toFixed(
      1
    )}% (threshold <= 0%)`,
    description:
  ...
  flags.push({
    ...
    threshold: "< 0.8",
    logic: `OCF / Total Net Profit = ${(ocf / netProfit).toFixed(
      2
    )} (threshold < 0.8)`,
    description:
  ...
  flags.push({
    ...
    threshold: "> 10%",
    logic: `Annualized (Total NI - OCF) / Total Assets = ${(
      accrualAssetsRatio * 100
    ).toFixed(1)}% (threshold > 10%)`,
    description:
  ...
  flags.push({
    ...
    threshold: "> 50%",
    logic: `Short-term Debt / Total Debt = ${(debtCliffRatio2 * 100).toFixed(
      1
    )}% (threshold > 50%)`,
    description:
  ...
  flags.push({
    ...
    logic: `Gross Margin: ${(grossMargin * 100).toFixed(1)}% (prev: ${(
      prevGrossMargin * 100
    ).toFixed(1)}%) & Operating Margin: ${(opMargin * 100).toFixed(
      1
    )}% (prev: ${(prevOpMargin * 100).toFixed(1)}%)`,
    description:
  ...
  flags.push({
    ...
    value: `Current: ${formatLargeNumber(annFCF)}, Prev: ${formatLargeNumber(
      annPrevFCF
    )}`,
    threshold: "Negative for > 1 period",
    logic: `Annualized Current FCF = ${formatLargeNumber(
      annFCF
    )} & Annualized Previous FCF = ${formatLargeNumber(
      annPrevFCF
    )} (threshold both < 0)`,
    description:
