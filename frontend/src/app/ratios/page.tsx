...
const calculateMetrics = (
  ...
  const financeExp =
    getVal(
      currentData,
      "income_statement.total_operating_cost.financial_expenses.amount"
    ) ||
    getVal(
      currentData,
  ...
  const netIncomeAttrParent = getVal(
    currentData,
  ...
  const begininterestpayable = prevData
    ? getVal(
        prevData,
  ...
  const prevInterestBearingDebt = prevData
    ? prevShortTermDebt + prevLongTermDebt
    : interestBearingDebt;

  const avgAssets =
    (totalAssets +
      (prevData
        ? getVal(prevData, "balance_sheet.assets_summary.total_assets")
        : totalAssets)) /
    2;
  const avgEquity =
    (totalEquity +
      (prevData
        ? getVal(prevData, "balance_sheet.equity.total_equity")
        : totalEquity)) /
    2;
  const avgInventory =
    (inventory +
      (prevData
        ? getVal(prevData, "balance_sheet.current_assets.inventories")
        : inventory)) /
    2;
  ...
  const avgPayables =
    (payables +
      (prevData
        ...
        : payables)) /
    2;
  const avgMonetaryFunds =
    (monetaryFunds +
      (prevData
        ? getVal(prevData, "balance_sheet.current_assets.monetary_funds")
        : monetaryFunds)) /
    2;
  const avgInterestBearingDebt =
    (interestBearingDebt + prevInterestBearingDebt) / 2;
  ...
  // ==========================================
  if (prevData) {
    const prevRevenue = getVal(
      prevData,
      "income_statement.total_operating_revenue"
    );
    const prevNetIncome = getVal(
      prevData,
      "income_statement.net_profit.amount"
    );
    const prevOCF = getVal(
      prevData,
      "cash_flow_statement.operating_activities.net_cash_flow_from_operating"
    );
...
const calculateDupont = (
  ...
  if (!currentReport)
    return { roe: 0, netMargin: 0, assetTurnover: 0, equityMultiplier: 0 };
  ...
  const preEquity = prevReport
    ? getVal(prevReport.data, "balance_sheet.equity.total_equity")
    : curEquity;
...
export default function KeyRatiosPage() {
  ...
  useEffect(() => {
    ...
    if (comparisonMode === "Target" && targetReports.length > 0) {
      comparisonReport = targetReports.find(
        (r) => r.fiscal_year === selectedYear
      );
      if (comparisonReport) {
        const targetPrevReport = targetReports.find((r) => {
          const rYearNum = parseInt(r.fiscal_year.split(" ")[0]);
          return (
            rYearNum === currentYearNum - 1 &&
            r.period_type === currentPeriodType
          );
        });
    ...
    const finalMetrics = currentMetrics.map((m) => {
      ...
      if (comp) {
        ...
        if (m.value.includes("%")) {
          ...
          changeStr = formatLargeNumber(diff);
        }
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2 group relative">
                            <span className="font-medium text-gray-900 text-sm cursor-help">
                              {metric.name}
                            </span>
                            {metric.formula && (
