...
export default function DataPage() {
  ...
  const [selectedTypes, setSelectedTypes] = useState<Set<string>>(
    new Set(["Income Statement", "Balance Sheet", "Cash Flow"])
  );
  ...
  const flattenReports = useCallback(
    (reports: any[]) => {
      ...
      reports.forEach((report: any) => {
        ...
        if (
          report.period_type &&
          report.period_type.toLowerCase() === "annual"
        ) {
        ...
        const processSection = (
          ...
          if (
            sectionName === "income_statement" &&
            sectionData.other_operating_income
          ) {
            ...
            if (!ooi.amount || ooi.amount === 0) {
              ...
              const sum =
                (ooi.fair_value_change_income || 0) +
            ...
            for (const key in obj) {
              ...
              const processItem = (val: number | any) => {
                if (!flattened[uniqueId]) {
                  ...
                  if (
                    typeLabel === "Cash Flow" &&
                    (accountName === "Credit Impairment Loss" ||
                      accountName === "信用减值损失")
                  ) {
                    itemLevel = 4;
                  }
                ...
                flattened[uniqueId][yearKey] =
                  typeof val === "number" ? val : val?.amount || 0;
              };
              ...
              if (typeof value === "object" && value !== null) {
                ...
                traverse(
                  value,
                  prefix ? `${prefix}.${key}` : `${sectionName}.${key}`
                );
    ...
    if (storedReports) {
      try {
        ...
        if (Array.isArray(reports) && reports.length > 0) {
          const allYearsRaw = Array.from(
            new Set(
              reports.map((r: any) => {
                let y = r.fiscal_year.trim();
                if (
                  r.period_type &&
                  r.period_type.toLowerCase() === "annual" &&
                  !y.toLowerCase().includes("annual")
                ) {
                  y = `${y} Annual`;
                }
                return y;
  ...
  useEffect(() => {
    ...
    const itemsList = flattened.map((r) => ({
      id: r.id,
      name: r.account,
      type: r.type,
    }));
  ...
  const filteredData = useMemo(() => {
    return displayRows.filter((row) => {
      ...
      if (hideZeros) {
        const allZeros = filteredYears.every(
  ...
  if (displayRows.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4">
        ...
        <Link
          href="/import"
          className="px-6 py-2 bg-primary text-white rounded-lg font-medium hover:bg-emerald-900 transition-colors flex items-center"
        >
    ...
    >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse table-auto">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-sm text-gray-600">
                <th className="py-3 px-6 font-semibold w-[300px] min-w-[300px]">
                  {t("lineItem")}
                </th>
                <th className="py-3 px-6 font-semibold w-[150px] min-w-[150px]">
                  {t("type")}
                </th>
                {filteredYears.map((year) => (
                  <th
                    key={year}
                    className="py-3 px-6 font-semibold text-right whitespace-nowrap min-w-[120px]"
                  >
                    {year}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.map((row, idx) => {
                const showGap =
                  idx > 0 && row.type !== filteredData[idx - 1].type;
                const isFirst = idx === 0;
                  ...
                  <Fragment key={row.id}>
                    {(isFirst || showGap) && (
                      <tr className="bg-gray-50 border-y border-gray-200">
                        <td
                          colSpan={2 + filteredYears.length}
                          className="px-6 py-4 bg-gray-200/50"
                        >
                    ...
                    >
                      <td
                        className={clsx(
                          ...
                          row.itemLevel === 1
                            ? "font-extrabold pl-4 pt-10 pb-4 text-lg border-t-2 border-gray-300 uppercase tracking-wide"
                            : row.itemLevel === 2
                            ? "font-bold pl-6 pt-8 pb-3 text-base border-t border-gray-200"
                            : row.itemLevel === 3
                            ? "font-medium pl-12 py-2.5 text-gray-700"
                      ...
                      {filteredYears.map((year) => (
                        <td
                          ...
                          className={clsx(
                            ...
                            row.itemLevel === 1
                              ? "pt-10 pb-4 font-extrabold text-gray-900 text-lg border-t-2 border-gray-300"
                              : row.itemLevel === 2
                              ? "pt-8 pb-3 font-semibold text-gray-900"
                              : row.itemLevel === 3
                              ? "py-2.5 text-gray-800"
                              : "py-1.5 text-gray-500 text-sm"
                          )}
                        >
                          {row[year] !== undefined
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 text-xs text-gray-500 flex justify-between">
          <span>
            {t("showingRecords")} {filteredData.length}
          </span>
